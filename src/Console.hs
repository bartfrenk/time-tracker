...
newHandle config tracker =
  Console.Handle {run = \args -> runReaderT (runM tracker args) config}

runM :: (MonadIO m, MonadReader Config m) => Tracker.Handle -> [Text] -> m ()
runM tracker args = do
  config <- ask
  command <- runParser config (expand (aliases config) args)
  process tracker command

process :: MonadIO m => Tracker.Handle -> Command -> m ()
process _ Version                     = printVersion
process tracker (Search jql)          = search tracker jql
